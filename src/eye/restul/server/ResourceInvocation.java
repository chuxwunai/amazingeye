...
public class ResourceInvocation {
	...
	public Object invoke() throws Throwable {
		try {
			...
			setRoute(route);
			//解析参数
			List<VariableExtractor> variableExtractors = this.buildVariableExtractors(route);
			int len = variableExtractors.size();
			for (int i = 0; i < route.getParameters().size(); i++) {
				if (i >= len) {
					setParameter(i, null);
				} else {
					setParameter(i, new LazyReference<Object>(new VariableGetter(this, variableExtractors.get(i))));
				}
			}
	...
	private static class VariableGetter implements Callable<Object> {

		private ResourceInvocation invocation;

		private VariableExtractor extractor;

		public VariableGetter(ResourceInvocation invocation, VariableExtractor extractor) {
		...
		public Object call() throws Exception {
			if (extractor == null) {
				System.out.println("参数解析器未定义，将传递 null 值。" + invocation.getRoute());
				return null;
			}
			return extractor.extract(invocation);
		}

	}

	private List<VariableExtractor> buildVariableExtractors(Route route) {
		List<VariableExtractor> variableExtractors = new ArrayList<VariableExtractor>();
		List<Parameter> parameters = route.getParameters();
		for (int i = 0; i < parameters.size(); i++) {
			String name = parameters.get(i).getName();
			Type type = parameters.get(i).getType();
			VariableExtractor variableExtractor = new DefaultVariableExtractor(name, type);
